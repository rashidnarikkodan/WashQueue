...
export default function Header({ role }: { role?: string }) {
  ...
  const currentRole =
    (role?.toLowerCase() as "admin" | "manager" | "owner" | "customer") || "customer"
    ...
    <header className="fixed top-1 z-40 w-full rounded-[3rem] border-b border-x border-border bg-card/90 backdrop-blur-md transition-all duration-300 shadow-md">
      <div className="mx-auto w-full px-6 py-3.5 grid grid-cols-3 items-center">
        ...
        <div className="col-span-1 flex items-center gap-3">
          ...
          {activeBadge && (
            <span
              className={`hidden lg:inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-semibold tracking-wide transition-all duration-300 ${activeBadge.className}`}
            >
              {activeBadge.label}
            </span>
          )}
