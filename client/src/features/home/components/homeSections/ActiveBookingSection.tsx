      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8 text-left">
        <div className="space-y-3">
          ...
          <p className="text-sm md:text-base text-muted-foreground max-w-xl">
            Estimated wait time:{" "}
            <span className="text-[#ADC6FF] font-bold">{data.activeBooking.estimatedWait}</span>.
            We'll notify you when{" "}
            <span className="font-bold text-foreground">{data.activeBooking.bayInfo}</span> is ready
            for your{" "}
            <span className="font-bold text-foreground">{data.activeBooking.vehicleName}</span>.
          </p>
        </div>
