...
export const authApi = {
  login: asyncHandle(async (email: string, password: string): Promise<AuthUser> => {
    const response = await api.post(API_ROUTES.AUTH.LOGIN, { email, password }, { skipToast: true })
    return unwrap(response.data)
  }, "Failed to login"),

  loginWithGoogle: asyncHandle(async (token: string): Promise<AuthUser> => {
    const response = await api.post(API_ROUTES.AUTH.GOOGLE, { token }, { skipToast: true })
    return unwrap(response.data)
  }, "Google Sign-In failed"),

  signup: asyncHandle(async (name: string, email: string, password: string): Promise<void> => {
    await api.post(
  ...
  verifyOTP: asyncHandle(async (email: string, code: string): Promise<AuthUser> => {
  ...
  me: asyncHandle(async (): Promise<AuthUser> => {
    const response = await api.get(API_ROUTES.AUTH.ME, { skipToast: true })

    return unwrap(response.data)
  }, "Failed to fetch user session"),

  logout: asyncHandle(async (): Promise<void> => {
    await api.post(API_ROUTES.AUTH.LOGOUT, {}, { skipToast: true })
  }, "Logout failed"),

  forgotPassword: asyncHandle(async (email: string): Promise<void> => {
    await api.post(
  ...
  resendOTP: asyncHandle(async (email: string): Promise<void> => {
    await api.post(
