...
const UserTable = ({
  ...
  const handleConfirmToggle = async () => {
    if (!pendingToggleUser || !onToggleStatus) return;
    await onToggleStatus(pendingToggleUser.id);
                      ...
                      <>
                        ...
                        <td className="py-4 px-6 text-right">
                          <div className="flex items-center justify-end gap-2">
                            {/* Hide block button entirely for admin accounts */}
                            {onToggleStatus && user.role !== ROLE.ADMIN && (
                              <button
                                onClick={() => handleToggleClick(user)}
                                title={!user.isBlocked ? "Block User" : "Activate User"}
                                className={`p-2 rounded-lg border transition-all cursor-pointer ${!user.isBlocked
