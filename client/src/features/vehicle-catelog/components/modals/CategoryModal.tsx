...
import type { VehicleCategory, CreateCategoryInput, UpdateCategoryInput } from "../../types"
...
export default function CategoryModal({ isOpen, onClose, onSave, category }: CategoryModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) {
      ...
      newErrors.name = "Category name must be under 100 characters"
    }
      ...
      <div className="flex flex-col p-6 space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-left">
          ...
          <FormInput
            ...
            placeholder="e.g. Passenger Car, Two Wheeler"
            disabled={isSubmitting}
            error={errors.name}
            required
          />
          ...
          <FormInput
            label={`URL Slug ${!isSlugManual && name.trim() ? "(Auto-generated)" : ""}`}
            type="text"
            value={slug}
