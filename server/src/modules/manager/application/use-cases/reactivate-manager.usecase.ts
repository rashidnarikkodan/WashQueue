...
import { IReactivateManagerUseCase } from "../interfaces/manager-usecases.interface"

import { IOwnerRepository } from "@/modules/owner/domain/repositories/owner.repository"

export class ReactivateManagerUseCase implements IReactivateManagerUseCase {
  constructor(
    private readonly managerAssignmentRepository: IManagerAssignmentRepository,
    private readonly ownerRepository: IOwnerRepository
  ) {}

  async execute(ownerUserId: string, assignmentId: string): Promise<ManagerAssignment> {
    ...
    if (!isOwner) {
      throw new ForbiddenError("You do not have permission to reactivate this manager assignment")
    }
