...
import { ISuspendManagerUseCase } from "../interfaces/manager-usecases.interface"

import { IOwnerRepository } from "@/modules/owner/domain/repositories/owner.repository"

export class SuspendManagerUseCase implements ISuspendManagerUseCase {
  constructor(
  ...
  async execute(ownerUserId: string, assignmentId: string): Promise<ManagerAssignment> {
    ...
    if (!isOwner) {
      throw new ForbiddenError("You do not have permission to suspend this manager assignment")
    }
