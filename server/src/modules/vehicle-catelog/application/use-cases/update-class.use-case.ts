  ...
  async execute(id: string, updates: UpdateClassInput): Promise<ClassResponseDto> {
    ...
    if (updates.name !== undefined && updates.name !== vehicleClass.name) {
      ...
      if (existingByName && existingByName.id !== vehicleClass.id) {
        throw new ConflictError(`Vehicle class "${updates.name}" already exists`)
      }
      ...
      if (existingBySlug && existingBySlug.id !== vehicleClass.id) {
        throw new ConflictError(`Vehicle class slug "${slug}" already exists`)
      }

      vehicleClass.rename(updates.name)
    }

    if (updates.description !== undefined && updates.description !== vehicleClass.description) {
      vehicleClass.changeDescription(updates.description)
    }

    if (updates.order !== undefined && updates.order !== vehicleClass.order) {
      vehicleClass.changeOrder(updates.order)
    }

    if (updates.isActive !== undefined && updates.isActive !== vehicleClass.isActive) {
